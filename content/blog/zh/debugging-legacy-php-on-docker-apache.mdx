---
title: "一個 nginx 使用者的跨棧遷移筆記：把 PHP 5.6 老專案搬進 Docker + Apache"
date: "2026-07-01"
description: "熟 nginx、幾乎沒碰過 Apache 與 PHP，卻要把跑在 CentOS 老主機上的 PHP 5.6 專案改成 Docker Compose、部署到新主機。沿著請求路徑一關一關除錯：反向代理、子路徑前綴、.htaccess 權限、容器連主機資料庫，最後收斂成一套可帶走的除錯方法論。"
tags: ["Apache", "PHP", "Docker", "Docker Compose", "反向代理", "舊系統遷移", "部署"]
category: "devops"
---

> 客戶要我把一個跑在 CentOS 老主機上的 PHP 5.6 專案，容器化成 Docker Compose、部署到新的 Linux 主機。難點不只在技術本身——PHP 5.6 早已 end-of-life、沒有測試保護，而我的主場是 React、TypeScript、Node.js、nginx、Docker，Apache2 與 PHP 幾乎沒碰過。這篇記錄我怎麼**把陌生技術對映到既有心智模型、沿著請求路徑系統性地定位問題**，最後不只讓它上線，還收斂出一套換到任何系統都能用的除錯方法。比起「我讓它動了」，這篇更想講「**我知道它為什麼動**」。
>
> 文中的網域、帳密、內部 IP 都已去識別化。假設你和我一樣熟 nginx、但沒碰過 Apache，也不熟 PHP。

先給趕時間的讀者一張全景：這是一次**正式環境的舊系統遷移**——把一個 EOL 的 PHP 5.6／mod_php 專案，從 CentOS 老主機搬進 `php:5.6-apache` 容器、用 Docker Compose 編排、跑在主機 Apache 的 TLS 反向代理後面；資料庫是主機上、與其他專案共用的正式 MariaDB；另外還有一個要掛在子路徑的 Next.js（Payload CMS）。整個「前端拿不到資源」的模糊症狀，最後拆出**四個分屬不同層級的根因**，而定位它們靠的不是猜，是一套沿著請求路徑、由外而內的系統性方法。
...
**為什麼卡在 5.6？** 通常不是不想升，而是升級成本高：程式碼裡散落著已被移除的函式與寫法，一升上去到處爆，而且往往沒有測試可以保護重構。對這種專案，**先「原封不動地容器化、讓它在現代主機上跑起來」，比「一邊搬一邊改寫」更務實**——這也是我這次刻意選的第一步：把「能跑」和「升級」拆成兩件事，一次只承擔一種風險。
...
**其二，容器 Apache：用 `Alias` 把 URL 前綴對映到 app 目錄。** **我刻意不去改動官方 image，而是把設定掛進會被載入的 `conf-enabled/`**（記取第一關的教訓）——最小侵入、可重現，下次換版也不會遺失：
...
修 bug 之外，這個專案真正花心思的，是幾個「當下沒人逼我、但我刻意這樣選」的決定。把它們單獨拎出來：

- **先容器化、不急著升版。** 面對 EOL 的 PHP 5.6 加上沒有測試保護，我選擇先原封不動搬進容器、讓它在現代主機上穩定跑，而不是邊搬邊改寫——把「能跑」和「升級」拆成兩件事，一次只承擔一種風險。
- **不改 image，改掛設定。** 前綴對映用 `Alias` 掛一份設定進容器的 `conf-enabled/`，而不是改動官方 image：最小侵入、可重現，換版也不會遺失。
- **除錯期用 `302`、定稿才換 `301`。** 預見到永久轉址會被瀏覽器頑固快取、反過來咬自己，先用暫時轉址把這個變因排除。
- **資料庫授權用萬用 `172.%`，不寫死 IP。** docker 指派的容器 IP 會變動，寫死等於埋一顆未來會爆的雷；用網段萬用字元一次到位。
- **驗證由內而外、一次一變數。** 先用 `curl` 直打容器繞過主機，把「主機 vs 容器」的戰線釘死，再往外測完整鏈——每步只動一個變數，壞了容易退。

這些不是事後補的漂亮話，是當下就決定「要不要動 image、要不要先升版、IP 要不要寫死」的實際選擇。
...
如果把這趟濃縮成「這是什麼樣的工程師會做的事」：在不熟悉的技術棧（Apache、PHP、mod_php）上快速建立心智模型、獨立扛起一個正式環境舊系統的容器化與部署、在 Linux 網路、反向代理、TLS、資料庫授權這些跨領域環節之間來回定位問題，並把過程整理成別人能複用的方法。前端、後端、維運這趟我都得自己走完——而我真正想證明的，不是我會用哪個工具，是遇到沒看過的系統時，我有一套穩定的方法把它拆開、弄懂、修好。

如果你對「接手別人的舊系統」這件事有興趣，我也寫過同一主題的不同切面：[優化舊專案存取效能：當問題不在程式，而在環境](/zh/blog/optimizing-legacy-system-performance) 與 [優化舊專案再思：從 Prompt 到 Context，再到 Harness](/zh/blog/rethinking-legacy-optimization-context-engineering)。
